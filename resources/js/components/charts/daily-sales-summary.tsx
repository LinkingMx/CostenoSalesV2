...
export function DailySalesSummary({
    ...
    } = useMainDashboardData(date, endDate);

    // Use API data if available, otherwise fall back to props
    const totalSales = apiData?.data?.sales?.total ?? propTotalSales ?? 0;
    const percentageChange = apiPercentageChange ?? propPercentageChange;
    const previousAmount = apiPreviousAmount ?? Math.round(totalSales * (1 - (propPercentageChange || 0) / 100));
    ...
    return (
        <Card className={`${className} overflow-hidden border-0 bg-transparent shadow-none`}>
            <CardContent className="px-4 pt-1 pb-1">
                ...
                <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-3">
                        <span className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{formatNumber(totalSales)}</span>
                        {percentageChange !== 0 && (
                            <div
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        ...
                        {apiPreviousAmount ? 'Mismo día semana anterior' : comparisonText}
                    </p>
                </div>
            </CardContent>
        </Card>
