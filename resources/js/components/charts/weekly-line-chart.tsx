...
export function WeeklyLineChart({ 
    ...
    const TitleWithIcon = () => {
        // Use the same data source as WeeklySalesSummary for consistency
        // Get totals from the main API call instead of summing individual days
        const { data: currentWeekApiData } = useMainDashboardData(startDate, endDate);
        ...
        const { data: previousWeekApiData } = useMainDashboardData(previousWeekStart, previousWeekEnd);
        
        // Get totals from API calls for the full periods (same as WeeklySalesSummary)
        const currentTotal = currentWeekApiData?.data?.sales?.total || 0;
        const previousTotal = previousWeekApiData?.data?.sales?.total || 0;
