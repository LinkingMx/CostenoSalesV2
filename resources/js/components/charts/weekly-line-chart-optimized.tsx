...
interface WeeklyLineChartOptimizedProps {
    chartData: WeeklyChartData[] | null;
    isLoading?: boolean;
    error?: Error | null;
    onRefresh?: () => void;
    className?: string;
 ...
 * - Coordinated loading state with other weekly components
...
export function WeeklyLineChartOptimized({ 
    ...
    const TitleWithIcon = () => {
        ...
        const currentTotal = chartData?.reduce((sum, day) => sum + day.current, 0) || 0;
        const previousTotal = chartData?.reduce((sum, day) => sum + day.previous, 0) || 0;
                ...
                <CardTitle className="flex items-center gap-2 text-xl font-semibold text-slate-900 dark:text-slate-100">
                    <Popover>
                        ...
                        <PopoverContent side="right" className="max-w-xs p-3">
                            <p className="text-sm">
                                <strong>Análisis Semanal:</strong> Compara el rendimiento día a día (Lunes a Domingo) de la semana seleccionada vs la semana anterior. 
                                Detecta tendencias semanales, identifica días de mayor/menor actividad y evalúa el crecimiento semanal.
                            </p>
                        </PopoverContent>
                    </Popover>
                    Análisis semanal
