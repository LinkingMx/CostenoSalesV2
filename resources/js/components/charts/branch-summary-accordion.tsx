...
interface BranchSummaryAccordionProps {
    date: string;
    endDate?: string;
    className?: string;
...
export function BranchSummaryAccordion({ date, endDate, className, onBranchDetailsClick }: BranchSummaryAccordionProps) {
    const { data, loading, error, comparisonData } = useMainDashboardData(date, endDate);
    const [expandedBranches, setExpandedBranches] = useState<Set<string>>(new Set());
    
    // Check if today is included in the date range to show/hide "Abiertas" section
    const showOpenAccounts = isTodayIncludedInRange(date, endDate || date);
