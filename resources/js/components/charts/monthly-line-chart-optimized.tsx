...
export function MonthlyLineChartOptimized({ 
    ...
    const TitleWithIcon = () => {
        ...
        const currentTotal = chartData?.reduce((sum, week) => sum + week.current, 0) || 0;
        const previousTotal = chartData?.reduce((sum, week) => sum + week.previous, 0) || 0;
                ...
                <CardTitle className="flex items-center gap-2 text-xl font-semibold text-slate-900 dark:text-slate-100">
                    <Popover>
                        ...
                        <PopoverContent side="right" className="max-w-xs p-3">
                            <p className="text-sm">
                                <strong>Análisis Mensual:</strong> Divide cada mes en semanas (S1-S5) y compara semana por semana vs el mes anterior. 
                                Analiza el progreso mensual, identifica semanas de alto/bajo rendimiento y evalúa patrones estacionales mensuales.
                            </p>
                        </PopoverContent>
                    </Popover>
                    Análisis mensual
    ...
    const CustomTooltipContent = ({ active, payload }: { active?: boolean; payload?: any[] }) => {
        ...
        const formatDateRange = (startDate?: string, endDate?: string) => {
            ...
            try {
                ...
                // If same month, show "1 - 7 ene", otherwise "30 dic - 5 ene"
                    ...
                    <div className="mt-1 space-y-0.5">
                        {formatDateRange(data.startDate, data.endDate) && (
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                <span className="font-medium">Actual:</span> {formatDateRange(data.startDate, data.endDate)}
                            </p>
                        )}
                        {formatDateRange(data.previousStartDate, data.previousEndDate) && (
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                <span className="font-medium">Anterior:</span> {formatDateRange(data.previousStartDate, data.previousEndDate)}
                            </p>
                        )}
                    </div>
                </div>
